...
public class ReductionMethod extends SolutionMethod {
	...
	protected void resolveUsingFractions() {
		...
		this.addTwoLastEquations(names);
		this.clearIncognitoLastEquationUsingFractions(names[1]);
		this.seStolution(names[1], this.getLastEquation(1));
		this.applyFirstEquationWithLastEquationUsingFractions(names);
		this.clearIncognitoLastEquationUsingFractions(names[0]);
		this.seStolution(names[0], this.getLastEquation(1));
	}
	...
	private void clearIncognitoLastEquationUsingFractions(String name) {
		this.copyLastEquation(1);
		this.getLastEquation(1).multiply(this.getLastEquation(2).getFractionValue(name).invert());
	}
	...
	private void applyFirstEquationWithLastEquationUsingFractions(String[] names) {
		this.copyLastEquation(9);
		this.getLastEquation(1).apply(names[1], this.getLastEquation(2).getFractionValue(Side.RIGHT));
		this.copyLastEquation(1);
		this.getLastEquation(1).add(new Constant(this.getLastEquation(2).getFractionValue(Side.LEFT).multiply(-1)));
		this.copyLastEquation(1);
		this.getLastEquation(1).simplify(Side.LEFT);
		this.copyLastEquation(1);
		this.getLastEquation(1).simplify(Side.RIGHT);
